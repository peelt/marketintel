...
export async function loadDividendDataset(
  ...
  const [securitiesRes, dividendsRes, pricesRes, financialsRes] =
    await Promise.all([
      supabase
        .from("securities")
      ...
      supabase
        ...
        .returns<FinancialsRow[]>(),
    ]);

  for (const [label, res] of [
    ["securities", securitiesRes],
    ["dividends", dividendsRes],
    ["prices", pricesRes],
    ["financials", financialsRes],
  ] as const) {
    if (res.error) {
      throw new Error(`loadDividendDataset ${label}: ${getErrorMessage(res.error)}`);
    }
  }

  const securities = new Map<string, SecurityRow>();
  for (const s of securitiesRes.data ?? []) securities.set(s.id, s);

  const dividends = groupBy(dividendsRes.data ?? [], (r) => r.security_id);
  const prices = groupBy(pricesRes.data ?? [], (r) => r.security_id);
