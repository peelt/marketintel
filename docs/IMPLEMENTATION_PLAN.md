...
| Scope reduction (26 Jul 2026) | **The product is the Reaction Analyser.** A live-data audit found the fundamentals-dependent signals structurally null in every filed edition (both markets — the source never populated the columns they read), Dividend strictly weaker than established dividend-safety services, IPO grading 4/25 prospectuses, and Metals/Geopolitical operationally disjoint from Reaction (≤2 names in its screen, zero co-filings). Dividend/Metals/IPO **retired**; Geopolitical **retired as a desk**, its macro read since rebuilt inside Reaction as the "why did it drop" backdrop (see the row below); Reaction framework v2 (migration 0015) drops its two dead fundamentals signals. Bar for any future desk: *intrinsic to Reaction*, not merely unique. Dividend re-entry condition: a fundamentals source completing its sustainability criterion for a defined (US-listed + UK-ADR) universe. Closes the London-fundamentals question: the gap was cross-market and source-structural, so no paid LSE source is warranted. | Whole product |
| Macro layer in Reaction (26 Jul 2026) | **Rebuild the retired Geopolitical desk's macro read inside Reaction as CONTEXT, not a signal.** One web-research call per run establishes the themes currently moving equity prices; they are injected into every per-name news call, which also attributes the drop (idiosyncratic / macro-amplified / macro-driven + theme). Informs the two grades the news call already returns rather than adding framework weight — so no framework v3, no migration, and the intel lens is untouched (classifications unchanged). The per-name *exposure* grading — the half that made the desk a second product — stays retired. Runs after the screen (calm days cost nothing); fail-soft to `unattributed`; echoed themes matched back to real ones so an invented theme never reaches evidence. Re-uses the retired desk's `## Macro read` markdown shape, so the existing accordion renderer picks it up unchanged. Next: audit filed editions before any weighting or surface follow-up. | Reaction |
